...
from mmrotate.core.bbox.coder.ellipse_coder import xywhaToellipse
...
class RotatedSingleStageDTLoss(nn.Module):
    def __init__(
        ...
        self.ellipse_loss = nn.SmoothL1Loss(reduction="none")
        self.loss_type = loss_type
        assert cls_loss_type in ["bce"]
        self.cls_loss_type = cls_loss_type
        if aux_loss:
            assert aux_loss in ["ot_loss_norm", "ot_ang_loss_norm"]
            self.ot_weight = aux_loss_cfg.pop("loss_weight", 1.0)
    ...
    def forward(
        ...
        if self.loss_type in ["pr_origin_p5"]:
            ...
            base_angs = torch.zeros(
                (len(teacher_preds), 1, 1024, 1024), device=gpu_device
            )
            for img_idx, bbox_result in enumerate(teacher_preds):
                    ...
                    if bbox_per_cls.shape[0] > 0:
                        bboxes.append(
                            np.hstack(
                                [
                                    bbox_per_cls,
                                    cls_idx * np.ones((bbox_per_cls.shape[0], 1)),
                                ]
                            )
                        )
                if len(bboxes) == 0:
                ...
                if len(valid_idx) == 0:
                    continue
                mask[img_idx] = xywha2rbox(
                    bboxes[valid_idx],
                    gpu_device,
                    img_meta=img_metas["unsup_strong"][img_idx],
                    ratio=self.rbox_pts_ratio,
                ).to(gpu_device)
            if self.loss_type in ["pr_origin_p5"]:
            ...
            raise RuntimeError(f"Not support {self.loss_type}")
        num_valid = (
            sum([_.sum() for _ in mask]) if isinstance(mask, list) else mask.sum()
        )
        if num_valid == 0:
            if self.apply_ot:
                return dict(
                    loss_raw=torch.tensor(0.0, device=teacher_logits[0][0].device),
                    loss_gc=torch.tensor(0.0, device=teacher_logits[0][0].device),
                )
            return dict(loss_raw=torch.tensor(0.0, device=teacher_logits[0][0].device))
        else:
            if self.loss_type in ["pr_origin_p5"]:
                t_cls_scores, t_bbox_preds, t_angle_preds, t_centernesses = (
                    teacher_logits
                )
                st_cls_scores, st_bbox_preds, st_ellipse_preds, st_centernesses = (
                    static_teacher_logits
                )

                s_cls_scores, s_bbox_preds, s_angle_preds, s_centernesses = (
                    student_logits
                )
                s_ellipse_preds = s_angle_preds

                target_size = (128, 128)
                t_cls_scores = F.interpolate(t_cls_scores[0], target_size).permute(
                    0, 2, 3, 1
                )[mask]
                t_bbox_preds = F.interpolate(t_bbox_preds[0], target_size).permute(
                    0, 2, 3, 1
                )[mask]
                t_angle_preds = F.interpolate(t_angle_preds[0], target_size).permute(
                    0, 2, 3, 1
                )[mask]
                t_centernesses = F.interpolate(t_centernesses[0], target_size).permute(
                    0, 2, 3, 1
                )[mask]

                st_bbox_preds = F.interpolate(st_bbox_preds[0], target_size).permute(
                    0, 2, 3, 1
                )[mask]
                st_ellipse_preds = F.interpolate(
                    st_ellipse_preds[0], target_size
                ).permute(0, 2, 3, 1)[mask]
                # st_ellipse_preds = torch.cat([st_bbox_preds[:,:2], st_ellipse_preds], dim=-1)

                s_cls_scores = F.interpolate(s_cls_scores[0], target_size).permute(
                    0, 2, 3, 1
                )[mask]
                s_bbox_preds = F.interpolate(s_bbox_preds[0], target_size).permute(
                    0, 2, 3, 1
                )[mask]
                s_angle_preds = F.interpolate(s_angle_preds[0], target_size).permute(
                    0, 2, 3, 1
                )[mask]
                s_centernesses = F.interpolate(s_centernesses[0], target_size).permute(
                    0, 2, 3, 1
                )[mask]

                s_ellipse_preds = F.interpolate(
                    s_ellipse_preds[0], target_size
                ).permute(0, 2, 3, 1)[mask]
            ...
            with torch.no_grad():
                if self.dynamic_weight in ["ang", "10ang", "50ang", "100ang"]:
                    ...
                    # [-pi, pi] -> [0, pi] -> [0, 1] -> [1, 2]
                    loss_weight = (
                        torch.abs(t_bbox_preds[:, -1] - s_bbox_preds[:, -1]) / np.pi
                    )
            ...
            loss_centerness = F.binary_cross_entropy(
                s_centernesses.sigmoid(), t_centernesses.sigmoid(), reduction="none"
            )
            ...
            loss_ellipse = (loss_ellipse * loss_weight * 0.1).mean()

            unsup_losses = dict(
                loss_raw=self.dynamic_fix_weight
                * (loss_cls + loss_bbox + loss_centerness + loss_ellipse)
            )
            if self.aux_loss:
                ...
                if self.loss_type in ["pr_origin_p5"]:
                    ...
                    assert (
                        t_score_map.shape[0] == 1
                    ), f"Only support batch size equals to 1 for now."
                    if teacher_logits[0][0].shape[-2:] != mask.shape[-2:]:
                        t_score_map = F.interpolate(
                            t_score_map, mask.shape[-2:]
                        ).permute(0, 2, 3, 1)
                        s_score_map = F.interpolate(
                            s_score_map, mask.shape[-2:]
                        ).permute(0, 2, 3, 1)
                    else:
                    ...
                    for img_idx in range(t_score_map.shape[0]):
                        t_score, score_cls = torch.max(
                            t_score_map[img_idx][mask[img_idx]], dim=-1
                        )
                        s_score = s_score_map[img_idx][mask[img_idx]][
                            torch.arange(t_score.shape[0], device=gpu_device), score_cls
                        ]
                    ...
                    unsup_losses.update(
                        loss_gc=self.ot_weight * loss_gc.sum() / t_score_map.shape[0]
                    )
                else:
                    raise RuntimeError(f"Not support {self.loss_type}")
