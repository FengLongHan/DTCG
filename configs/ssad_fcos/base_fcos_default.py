from configs.rotated_fcos.ellipse_fcos_fpn_le90 import model as detector
from configs.rotated_fcos.ellipse_fcos_fpn_le90 import angle_version
...
custom_imports = dict(imports=["ssad"], allow_failed_imports=False)
...
unsup_pipeline = [
    ...
    dict(
        type="STMultiBranch",
        unsup_strong=deepcopy(strong_pipeline),
        unsup_weak=deepcopy(weak_pipeline),
...
data = dict(
    ...
    val=src_data["val"],
    test=src_data["test"],
    sampler=dict(train=dict(type="GroupMultiSourceSampler", sample_ratio=[2, 1])),
)
